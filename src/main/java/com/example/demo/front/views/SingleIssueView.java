...
public class SingleIssueView extends AbstractLayout implements HasUrlParameter<Long> {
    ...
    public SingleIssueView(IssueService issueService, UserService service) {
        ...
        provider.addDataProviderListener(dataChangeEvent ->
                commentList.forEach(comment -> comments.add(new CommentDiv(comment))));
    ...
    public void setParameter(BeforeEvent event, Long parameter) {
        ...
        commentList.addAll(issueService.findIssueComments(parameter));
        provider.refreshAll();

        if (SecurityUtil.isAuthenticated()) {
            commentFormDiv.add(new CommentForm(parameter, provider, issueService));
        }
